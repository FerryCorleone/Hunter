...
本轮按 `vibe-product-builder -> PRD 页面契约 -> imagegen-to-html-design -> SwiftUI 实现` 重新走设计开发流程。新参考图已保存到：
...
- Settings Window：左侧 196px sidebar、右侧 760px max-width 单列设置列表、白色实体 settings row、低饱和蓝色选中态。
...
- 左侧 sidebar 固定宽度，导航项整行可点，选中态使用低饱和蓝色背景。
- 右侧内容限制最大阅读宽度，避免表单在大窗口里横向拉散。
- 每个设置项使用统一卡片：小号蓝色 SF Symbol、左侧标题说明、右侧控件区。
