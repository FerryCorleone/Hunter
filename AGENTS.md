...
- ASR 支持本地模型模式，用户可在设置页下载 `SenseVoice Small INT8 (sherpa-onnx)` 到本机；TTS 本地模型方案已否掉，产品内不再提供本地 TTS 下载、Qwen worker 或声音克隆本机样本入口。
- 内置推荐配置仅作为模板。当前默认测试链路为本地 SenseVoice ASR -> DeepSeek `deepseek-v4-flash` -> 云端 CosyVoice `cosyvoice-v3-flash + longanyang`。抓包/对喷播报不得静默降级到 macOS 系统朗读；TTS 失败必须在状态和诊断日志里明确报错。
...
- macOS 修复可以在完成验证后按用户要求合进 `main`。
- Windows 相关改动在用户明确确认“上线发布 Windows 版本”之前，保持为未发布工作；不要把 Windows 构建、安装包、Release asset 或上线说明发布出去。
- 处理 macOS bugfix、文档或发布时，不能顺手夹带 `windows/`、Windows CI、Windows packaging 或 Windows parity 改动，除非用户明确把这些文件列入本次范围。
- 如果 Windows 工作已经达到可编译或 CI 通过，也只能描述为待真机验收或 manual-QA-ready，不能直接宣称已经上线或可正式发布。
