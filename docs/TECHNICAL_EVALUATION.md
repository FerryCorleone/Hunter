...
当前本机测试链路调整为 **DeepSeek LLM + 本地 ASR/TTS 优先**：
...
- 本地 TTS 首选：`Qwen3-TTS-12Hz-0.6B-CustomVoice`，支持中文/英文等 10 种语言、9 个预置音色和自然语言语气控制。
- 本地声音克隆：`Qwen3-TTS-12Hz-0.6B-Base`，只在用户选择克隆声音并提供授权声音样本时使用。
- 云端 ASR/TTS 仍保留阿里百炼模板：`paraformer-realtime-v2`、`cosyvoice-v3-flash + longanyang`。
- Web Search 首选：Brave Search API；Tavily 作为“AI-ready 搜索摘要”备选。

理由：DeepSeek 的文本接口符合 OpenAI Chat Completions 形态，接入成本低；ASR/TTS 在 Mac 本地运行可以减少日常语音成本和隐私压力；Brave 的 raw search snippets 足够给 LLM 做页面语境增强，成本低于 Tavily；云端 API 入口继续保留给不想下载模型或追求低延迟托管服务的用户。
...
- 本地部署：ASR/TTS 支持用户在设置页选择“本地模型”，并下载官方/开源模型到本机。
...
- TTS：默认 `Qwen3-TTS 0.6B CustomVoice` + `Qwen3-TTS-Tokenizer-12Hz`，下载到 `Application Support/Hunter/LocalModels/tts`；克隆模式另用 `Qwen3-TTS 0.6B Base`。
...
| 本地 Qwen3-TTS CustomVoice | 本地 TTS 默认 | 模型免费下载，本机推理 | 中英文、多语言、9 个预置音色、语气控制，Apache-2.0 | 模型较大，依赖 Python 3.12/本地 ML runtime；低配 Mac 延迟需继续压测 |
| 本地 Qwen3-TTS Base | 本地声音克隆 | 模型免费下载，本机推理 | 短参考音频声音克隆；不需要云端上传样本 | 需要用户授权声音样本；不适合作为无样本默认音色 |
...
本地首选：`Qwen3-TTS-12Hz-0.6B-CustomVoice`

- 用途：抓包播报和 AI 回怼。
- 能力：支持中文、英文等 10 种语言；CustomVoice checkpoint 支持 9 个预置 speaker 和自然语言语气控制。
- MVP 用法：预置音色不需要声音样本；用户选择克隆声音时，再切换到 Qwen3-TTS Base 并要求授权样本。

云端 fallback：阿里云百炼 `cosyvoice-v3-flash`
...
本地 ASR/TTS + DeepSeek LLM 估算：

- ASR：本地推理，云端成本 0。
- TTS：本地推理，云端成本 0。
- LLM：约 18,600 Token，按 DeepSeek 官方价格折算，通常会低于语音云服务成本。
- 注意：本地 TTS 的实际“成本”转为下载体积、磁盘占用和 Mac 推理延迟，需要用真实设备压测。
