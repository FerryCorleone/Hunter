...
- 默认云端 ASR：阿里百炼 `paraformer-realtime-v2`，保留 OpenAI `gpt-4o-mini-transcribe` 模板；用户先填写自己的 API Key。
...
- 云端 ASR/TTS 保留阿里百炼模板：`paraformer-realtime-v2`、`cosyvoice-v3.5-flash`；OpenAI 模板为 `gpt-4o-mini-transcribe` / `gpt-4o-mini-tts + coral`；MiMo TTS 模板为默认：`https://api.xiaomimimo.com/v1`、`mimo-v2.5-tts`、`MIMO_API_KEY`、`api-key` 鉴权头。
...
| Xiaomi MiMo | MVP 默认云端 TTS / inline 授权样本克隆 | 定价页显示 V2.5 TTS、VoiceClone、VoiceDesign 当前限时免费 | V2.5 TTS 支持预置音色、文本音色设计、inline 音频样本复刻，当前实现已接通，Chat Completions 形态便于接入 | 免费策略可能变；inline clone 每次请求携带授权样本，不是长期 `voice_id`；鉴权使用 `api-key` header；音频以 base64 返回；V2.5 流式目前为兼容模式 |
...
- 强制模式必须是用户主动选择的强度档，只关闭当前命中的浏览器标签页或请求退出当前前台 App；不做断网、强杀、不可关闭监督或远程控制。
- 用户可以自定义禁用词。
