...
| 本地 Qwen3-TTS | 本地 TTS 默认 | 模型免费下载，本机推理 | 中英文、多语言、短参考音频声音克隆，Apache-2.0 | 模型较大，依赖 Python 3.12/本地 ML runtime；低配 Mac 延迟需继续压测 |
...
- MVP 用法：提供模型下载、授权样本上传/录制、本机路径保存和 Qwen3-TTS 克隆 worker 调用路径；若用户未配置授权样本或模型未就绪，运行时不上传音频，改用 macOS 系统语音本地降级。
