...
| TTS 默认链路 | Pass | 本地默认 TTS 使用 Qwen3-TTS CustomVoice 预置音色，默认 speaker 为 `Vivian`，不要求声音样本；`--smoke-local-tts` 已生成 24kHz WAV，`tts.log` 记录 `LOCAL_TTS_START` 和 `LOCAL_TTS_SUCCESS`；克隆声音另走 Base + 授权样本 |
