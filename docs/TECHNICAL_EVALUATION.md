...
- 本地 TTS 首选：`Qwen3-TTS-12Hz-0.6B-Base`，支持中文/英文等 10 种语言和短参考音频声音克隆。
- 云端 ASR/TTS 仍保留阿里百炼模板：`paraformer-realtime-v2`、`cosyvoice-v3-flash + longanyang`。

理由：DeepSeek 的文本接口符合 OpenAI Chat Completions 形态，接入成本低；ASR/TTS 在 Mac 本地运行可以减少日常语音成本和隐私压力；云端 API 入口继续保留给不想下载模型或追求低延迟托管服务的用户。
...
- TTS：`Qwen3-TTS 0.6B Base` + `Qwen3-TTS-Tokenizer-12Hz`，下载到 `Application Support/Hunter/LocalModels/tts`。
...
| 本地 SenseVoice | 本地 ASR 默认 | 模型免费下载，本机推理 | 中英日韩粤，sherpa-onnx 有 Swift/macOS 路线，隐私和成本好 | 标点和口语纠错需后处理；需要接本地推理 adapter |
| 本地 Qwen3-TTS | 本地 TTS 默认 | 模型免费下载，本机推理 | 中英文、多语言、短参考音频声音克隆，Apache-2.0 | 模型较大，Mac 低配机器延迟需实测；需要本地 worker/adapter |
...
- MVP 用法：先提供模型下载和路径管理；下一步接入本地 ASR adapter。
...
本地首选：`Qwen3-TTS-12Hz-0.6B-Base`

- 用途：抓包播报和 AI 回怼。
- 能力：支持中文、英文等 10 种语言；支持短参考音频声音克隆。
- MVP 用法：先提供模型下载、授权样本上传/录制和本机路径保存；下一步接入本地 TTS worker/adapter。
