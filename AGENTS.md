...
- 内置推荐配置仅作为模板。当前默认测试链路为本地 SenseVoice ASR -> DeepSeek `deepseek-v4-flash` -> 本地 Qwen3-TTS CustomVoice 预置音色；克隆声音另走 Qwen3-TTS Base + 用户授权样本。抓包/对喷播报不得静默降级到 macOS 系统朗读；TTS 失败必须在状态和诊断日志里明确报错。云端 ASR/TTS fallback 保留阿里百炼链路：`paraformer-realtime-v2 -> cosyvoice-v3-flash`。
