...
| 权限引导 | Pass | 设置页展示麦克风、浏览器自动化、通知和辅助功能（可选）状态；麦克风可请求或打开系统设置，浏览器自动化优先检查当前浏览器，若设置窗口在前台则回退检查 Chrome，且只在用户点击授权时触发系统弹窗；通知可请求授权；按钮改为自适应宽度，避免文字裁切；本机辅助功能和麦克风均已允许 |
