...
| 单元测试通过 | Pass | `swift test`，6 个测试覆盖时长解析与工作时段 |
...
| 工作时段配置 | Pass | 设置页可启用开始/结束时间，单测覆盖日间和跨午夜窗口 |
| 网站/App 黑名单配置 | Pass | 设置页支持新增、删除、启用/停用规则 |
...
| Chrome/Safari URL 检测 | Partial | 代码使用 AppleScript；需浏览器自动化授权后验收 |
| 安全与隐私 | Pass | `.env.local` 被忽略，API Key 可写 Keychain，仓库未发现明文 key |
| 音色克隆 | Not in MVP | 需要用户授权样本或选择声音设计方案 |
| 登录时启动 | Not in MVP | UI 占位，后续接 `SMAppService` |
